...
function getJIRAByID(next) {
  jira
    ...
    .then(function(issue) {
      ...
      if (!issue.fields[conf.SERVICES]) {
       return next(new Error('Please add at least one impacted service to the JIRA'));
      }
      var ctx = {
        ID: ID,
        project: project,
        ...
        impact: issue.fields[conf.IMPACT_TXT],
...
function updateStatusNotice(ctx, next) {
  ...
  requestBody.incident.status = ctx.conf.INCSTATUS_MAP[ctx.incident_status];
