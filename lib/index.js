...
function validateJIRA(ctx, next) {
  ...
  request.get({
    ...
    headers: ctx.statusio.headers
      ...
      // Check Component for "Compute Infrastructure"
      if (ctx.issue.issuetype.name === 'Compute Infrastructure') {
        WARN('VERIFY', 'You have used type Compute Infrastructure. For individual server faults the notices must ONLY be sent to the affected customers via sup-notify. im-notices should only be used if the impact affects all, or a large number of, customers. If you still wish to use im-notices then reply "Yes" to this prompt and then enter your reasons, which will be added as a comment to the JIRA.');
        readline.question('Enter response: ', function (answer) {
          if (answer !== 'Yes') {
            return next("Aborted due to component being 'Compute Infrastructure'");
          }
          readline.question('Enter reason for using im-notices for a Compute Infrastructure issue: ', function (answer) {
            ctx.issue.ci_reason = answer;
            next(null, ctx);
          });
        });
      } else {
